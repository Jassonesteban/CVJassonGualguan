  ...
  return (
    <>
      <section className="flex flex-col-reverse md:flex-row items-center justify-between mt-4">
        <motion.div
          className="flex flex-col space-y-4 max-w-full md:max-w-[50%]"
          initial={{ opacity: 0, x: -100 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 1, ease: "easeOut" }}
        >
          <h2
            className={`text-8xl font-extrabold text-left ${roboto.className}`}
          >
            {translations[lang].hello}
          </h2>
          <p
            className={`text-2xl font-normal text-gray text-justify ${roboto.className}`}
          >
        ...
        <motion.div
          className="relative w-56 h-56 md:w-56 md:h-56 lg:w-84 lg:h-84 mb-10"
          initial={{ opacity: 0, x: 100 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 1, ease: "easeOut" }}
        >
      ...
      <section className="mt-20">
        <h1
          className={`text-5xl text-left mb-10 md:text-3xl ${roboto.className}`}
        >
          {translations[lang].pila_tecnologica}
        </h1>
        <StackTechnology techs={data["habilidades_tecnicas"]} />
        <h5 className="text-2xl text-gray mt-5 mb-5">
          {translations[lang].mas}
        </h5>
      </section>
      <section className="mt-20">
